...
{
public:
    SendRecvKernel(uint32_t instance, xrt::device &device, xrt::uuid &xclbin_uuid, Configuration &config, std::vector<char> &data) : instance(instance), config(config)
    {
        char name[100];
        snprintf(name, 100, "send_recv:{send_recv_%u}", instance);
        kernel = xrt::kernel(device, xclbin_uuid, name);

        data_input_bo = xrt::bo(device, config.max_num_bytes, xrt::bo::flags::normal, kernel.group_id(2));

        data_input_bo.write(data.data());
        data_input_bo.sync(XCL_BO_SYNC_BO_TO_DEVICE);

        data_output_bo = xrt::bo(device, config.max_num_bytes, xrt::bo::flags::normal, kernel.group_id(3));

        data_output.resize(config.max_num_bytes);
    }
    ...
    void prepare_repetition(uint32_t repetition)
    {
        ...
        run.set_arg(4, config.iterations_per_message[repetition]);
        run.set_arg(5, config.message_sizes[repetition]);
        run.set_arg(6, config.frame_sizes[repetition]);
    }
    ...
    void write_back()
    {
        data_output_bo.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
        data_output_bo.read(data_output.data());
    }
    ...
    uint32_t compare_data(char *ref, uint32_t repetition)
    {
        uint32_t err_num = 0;
        for (uint32_t i = 0; i < config.message_sizes[repetition]; i++) {
            if (data_output[i] != ref[i]) {
                if (err_num < 16) {
                    printf("recv[%d] = %02x, send[%d] = %02x\n", i, (uint8_t)data_output[i], i, (uint8_t)ref[i]);
                }
                err_num++;
            }
        }
...
private:
    xrt::bo data_input_bo;
    xrt::bo data_output_bo;
    xrt::kernel kernel;
    xrt::run run;
    uint32_t instance;
    Configuration config;
...
public:
    ...
    {
        char name[100];
        snprintf(name, 100, "recv_send:{recv_send_%u}", instance);
        kernel = xrt::kernel(device, xclbin_uuid, name);
    }
